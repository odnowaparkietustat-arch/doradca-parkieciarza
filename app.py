...
if st.button("GENERUJ PROTOKÓŁ OGLĘDZIN", type="primary", use_container_width=True):
    if moisture is None: st.error("Proszę podać wilgotność podłoża!")
        ...
        if decision_after_cure == "Wykonanie bariery przeciwwilgociowej":
            if strength_val <= 2:
                st.write("* **Zalecamy wykonanie bariery przeciwwilgociowej poprzez dwukrotne zagruntowanie gruntówką wzmacniającą WAKOL PU 235.**\n1 - warstwa wałkiem ok. 150 g/m². Schnięcie 3-6h.\n2 warstwa ok. 100 g/m². Schnięcie 3-6h.")
            else:
                st.write("* **Zalecamy stworzenie bariery przeciwwilgociowej gruntówką WAKOL PU 280.**\n1 warstwa ok. 150 g/m². Schnięcie 1h.\n2 warstwa ok. 100 g/m². Schnięcie 1h.")
            if needs_levelling == "TAK": st.write(f"* **Następnie zaaplikować mostek sczepny WAKOL D 3045 (wałkiem, 150g/m2, 1h).**")
            ...
            if strength_val == 1:
                if needs_levelling == "NIE": 
                    ...
                    st.write(f"* {p}Zalecamy gruntowanie wzmacniające **WAKOL PU 235** (150g/m2).")
                    st.write(f"* **Następnie zaaplikować mostek sczepny WAKOL D 3045 (150g/m2, 1h).**")
            elif strength_val == 2:
                ...
                st.write(f"* {p}**Zagruntować podłoże koncentratem WAKOL D 3040 (1 część + 2 części wody, wałek microfazer, zużycie 50g/m2 koncentratu, schnięcie 30 min).**")

        # SEKCJA MAS WYRÓWNAWCZYCH - PEŁNE OPISY
        if needs_levelling == "TAK":
