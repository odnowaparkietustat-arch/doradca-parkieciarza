...
if st.button("GENERUJ PROTOKÓŁ OGLĘDZIN", type="primary", use_container_width=True):
    if moisture is None: st.error("Podaj wilgotność!")
        ...
        # LOGIKA NOWEJ FORMUŁY DLA OGRZEWANIA BEZ WYGRZEWANIA
        ...
        if decision_after_cure == "konieczność wykonania bariery przeciwwilgociowej":
            ...
            # W scenariuszu "bez wygrzewania" idziemy ścieżką jak dla suchego podłoża (po zakończeniu procesu)
            if strength_val == 1:
                st.write(FULL_PS275)
        ...
        if needs_levelling == "TAK":
            # Wyświetlamy masy, chyba że faktycznie jesteśmy w trakcie "bez wygrzewania" - ale zgodnie z poleceniem traktujemy to jako instrukcję "co po osuszeniu"
