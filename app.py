...
presso_results = []
for i in range(6): 
    presso_val = st.number_input(f"Próba {i+1} (N/mm²)", min_value=0.0, step=0.1, key=f"p_{i}", value=None)
    presso_results.append(presso_val)
...
if st.button("GENERUJ PROTOKÓŁ OGLĘDZIN", type="primary", use_container_width=True):
    if moisture is None: 
        ...
        if (klaw_meters + pek_meters) > 0 or holes == "TAK":
            ...
            if (klaw_meters + pek_meters) > 0: 
                st.write(f"  - Zespolić pęknięcia i dylatacje pozorne żywicą **WAKOL PS 205**.")
            if holes == "TAK": 
                st.write(f"  - Uzupełnić ubytki i zdegradowane fragmenty{hole_details} zaprawą szybkosprawną **WAKOL Z 610**.")
        ...
        if decision_after_cure == "Wykonanie bariery przeciwwilgociowej":
            ...
            if strength_val == 1:
                ...
                st.write(f"* {p}Zalecamy gruntowanie wzmacniające **WAKOL PU 280** (zużycie 150g/m2).")
                if needs_levelling == "TAK":
                    st.write(f"* **Następnie należy zaaplikować mostek sczepny za pomocą produktu WAKOL D 3045. Aplikacja wałkiem. Zużycie - 150 gr. Czas schnięcia - 1 godzina.**")
