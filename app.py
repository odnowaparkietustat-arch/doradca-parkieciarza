...
substrate_age_val = st.number_input("Wiek podłoża (miesiące):", min_value=0.5, step=0.5, format="%.1f", value=None, placeholder="Wpisz ilość miesięcy...")
...
if heating_exists == "TAK":
    ...
    if h_type in ["wodne klasyczne", "płyta fundamentowa grzewcza"]:
        st.write("❓ Czy został przeprowadzony proces wygrzewania?")
...
klaw_meters = 0.0
if cracks_klaw == "TAK": klaw_meters = st.number_input("Ilość mb klawiszujących:", min_value=0.1, step=0.1, value=None, placeholder="mb...")
st.write("7. Czy występują pęknięcia podłoża?")
cracks_pek = st.radio("Pęknięcia:", ["TAK", "NIE"], index=1, horizontal=True, label_visibility="collapsed")
pek_meters = 0.0
if cracks_pek == "TAK": pek_meters = st.number_input("Ilość mb pęknięć:", min_value=0.1, step=0.1, value=None, placeholder="mb...")
...
moisture = st.number_input("12. Poziom wilgoci (CM %)", format="%.1f", value=None)
...
# --- LOGIKA NORM ---
limit = 1.5 if (substrate == "jastrych cementowy" and heating_exists == "TAK") else 1.8 if substrate == "jastrych cementowy" else 0.3 if heating_exists == "TAK" else 0.5
...
if st.button("GENERUJ PROTOKÓŁ OGLĘDZIN", type="primary", use_container_width=True):
    if moisture is None: st.error("Podaj wilgotność!")
        ...
        heat_txt = f" Stwierdzono {heating_info}." if heating_exists == "TAK" else " Brak ogrzewania podłogowego."
        obw_txt = " Dylatacje obwodowe OK." if dilatations_obw_ok == "TAK" else " Dylatacje obwodowe NIEPRAWIDŁOWE."
        level_txt = f" Podłoże wymaga wyrównania ({leveling_thickness} mm)." if needs_levelling == "TAK" else " Brak wymogu wyrównania."
        st.write(f"**a) oględziny optyczne:** Podłoże: {substrate}{age_txt}. {heat_txt} {obw_txt} Pęknięcia: {'TAK' if cracks_pek=='TAK' else 'NIE'}. Klawiszowanie: {'TAK' if cracks_klaw=='TAK' else 'NIE'}. {level_txt}")
        
        valid_p = [v for v in presso_results if v is not None and v > 0]
        if valid_p:
            st.write(f"**Wynik badania PressoMess:** {', '.join([str(x) for x in valid_p])} N/mm²")
        ...
        if total_cracks := (klaw_meters + pek_meters):
            if moisture_prefix: st.write(f"* {moisture_prefix}")
            st.write(f"  - Zespolić pęknięcia żywicą **WAKOL PS 205**.")

        # --- PEŁNY OPIS PU 280 (BARIERA) ---
        if decision_after_cure == "Wykonanie bariery przeciwwilgociowej":
            ...
            if needs_levelling == "TAK": st.write("* **Następnie mostek sczepny WAKOL D 3045 (150g/m2, 1h).**")
        
        elif decision_after_cure not in ["dalsze osuszanie", "kolejny proces wygrzewania"]:
            p = moisture_prefix + " " if moisture_prefix else ""
            if strength_val == 1:
                if needs_levelling == "NIE": st.write(f"* {p}Zalecamy gruntowanie głębokie **Wakol PS 275** (2 warstwy po 350g/m2, szlif po 7 dniach).")
                else: st.write(f"* {p}Gruntowanie **WAKOL PU 235** + **mostek D 3045**.")
            elif strength_val == 2: st.write(f"* {p}Gruntowanie **WAKOL PU 280**.")
