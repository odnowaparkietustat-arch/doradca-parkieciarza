...
with col_w1: temp_air = st.number_input("10. Temperatura powietrza (°C)", step=0.5, value=None, placeholder="°C...")
with col_w2: hum_air = st.number_input("11. Wilgotność powietrza (%)", step=1.0, value=None, placeholder="% RH...")
moisture = st.number_input("12. Poziom wilgoci podłoża (CM %)", placeholder="Wynik CM...", format="%.1f", value=None)
...
if moisture is not None and moisture > limit:
    ...
    if heating_exists == "TAK" and heating_curing_done == "NIE": decision_after_cure = opt_dry
    else:
        if moisture <= barrier_max:
            if strength_val == 1 and needs_levelling == "NIE": decision_after_cure = opt_dry
            else: decision_after_cure = st.radio("Postępowanie:", ["Wykonanie bariery przeciwwilgociowej", opt_dry], horizontal=True)
...
if st.button("GENERUJ PROTOKÓŁ OGLĘDZIN", type="primary", use_container_width=True):
    if moisture is None: st.error("Proszę podać wilgotność podłoża!")
        ...
        # Opis optyczny (z informacją o wyrównaniu)
        ...
        curing_txt = " Przeprowadzono wygrzewanie." if heating_curing_done == "TAK" else " Brak protokołu wygrzewania." if heating_exists == "TAK" else ""
        obw_txt = " Dylatacje obwodowe OK." if dilatations_obw_ok == "TAK" else " Dylatacje obwodowe NIEPRAWIDŁOWE."
        klaw_txt = f" Klawiszowanie ({klaw_meters} mb)." if cracks_klaw == "TAK" else " Brak klawiszowania."
        pek_txt = f" Pęknięcia ({pek_meters} mb)." if cracks_pek == "TAK" else " Brak pęknięć."
        level_desc = f" Podłoże wymaga wyrównania ({leveling_thickness} mm)." if needs_levelling == "TAK" else " Podłoże nie wymaga wyrównania."
        st.write(f"**a) oględziny optyczne:** Podłoże: {substrate}{age_txt}. {heat_txt}{curing_txt} {obw_txt} {klaw_txt} {pek_txt} {level_desc} Wentylacja: **{ventilation_type}**.")
        ...
        if decision_after_cure == "Wykonanie bariery przeciwwilgociowej":
            if strength_val <= 2:
                st.write("* **Bariera przeciwwilgociowa: 2x WAKOL PU 235** (łącznie 250g/m2, schnięcie 3-6h/warstwa).")
            else:
                st.write("* **Bariera przeciwwilgociowa: 2x WAKOL PU 280** (łącznie 250g/m2, schnięcie 1h/warstwa).")
            if needs_levelling == "TAK": st.write(f"* **Następnie mostek sczepny WAKOL D 3045 (150g/m2, 1h).**")
        elif decision_after_cure not in ["dalsze osuszanie", "kolejny proces wygrzewania"]:
            p = moisture_prefix + " " if moisture_prefix else ""
            if strength_val == 1:
                if needs_levelling == "NIE": st.write(f"* {p}Zalecamy gruntowanie głębokie **Wakol PS 275** (2 warstwy po 350g/m2, odstęp 1h, szlif po 7 dniach).")
                else: st.write(f"* {p}Gruntowanie **WAKOL PU 235** + **mostek D 3045**.")
            elif strength_val == 2:
                st.write(f"* {p}Gruntowanie **WAKOL PU 280**.")
                if needs_levelling == "TAK": st.write(f"* **Następnie mostek sczepny WAKOL D 3045.**")
            elif strength_val >= 3 and needs_levelling == "TAK": st.write(f"* {p}**Zagruntować koncentratem WAKOL D 3040 (1:2 z wodą).**")
